...
2. `∀x, ∀i ∈ I, f(x)[i] = g(x)[i]` (其中 `I` 如定义 4.1 所示，包含了 `e_0, e_1, ..., e_n`)。

**结论:**
`τ_f ≃_P τ_g` (存在一个唯一的置换 `P` 使得 `∀i, P(τ_f(i)) = τ_g(i)`)。

**证明:**
1.  根据前提 2，对于索引子集 `I` 中的每一个标准基向量 `e_j` (`j` 从 0 到 `n`)，我们都有：
    `∀x, f(x)[e_j] = g(x)[e_j]`。
2.  根据定理 5.2 (依赖映射的置换唯一性)，对于每一个 `e_j`，都存在一个唯一的置换 `P_j`，使得：
    `P_j(τ_f(e_j)) = τ_g(e_j)`。
3.  根据线性依赖映射的定义 (2.4)，`τ(i)` 是一个由 `k` 个 `m` 维向量组成的列表: `τ(i) = [M·i + b_1, ..., M·i + b_k]`。我们可以将 `τ(e_j)` 展开：
    - `τ_f(e_j) = [M_f·e_j + b_{f,1}, ..., M_f·e_j + b_{f,k}]`
    - `τ_g(e_j) = [M_g·e_j + b_{g,1}, ..., M_g·e_j + b_{g,k}]`
4.  将步骤 3 代入步骤 2，我们得到 `n+1` 组等式，每组对应一个 `j ∈ {0, ..., n}`：
    `P_j([M_f·e_j + b_{f,1}, ..., M_f·e_j + b_{f,k}]) = [M_g·e_j + b_{g,1}, ..., M_g·e_j + b_{g,k}]`。
5.  **证明所有置换 `P_j` 都相等 (即 `P_0 = P_1 = ... = P_n`)**:
    - 根据 **引理 5.5 (置换等价性)** 的证明，我们可以得出结论：从上述 `n+1` 组等式中推导出的所有置换 `P_j` 都是相等的。
    - 我们将这个唯一的置换记为 `P`。
6.  现在我们证明对于任意索引 `i ∈ N^n`，都有 `P(τ_f(i)) = τ_g(i)`。
7.  任何索引 `i` 都可以表示为标准基向量的线性组合：`i = a_1*e_1 + ... + a_n*e_n` (忽略 `e_0` 因为 `M·e_0=0`)。
8.  根据线性依赖映射定义 (2.4)，我们有：
    - `τ_f(i) = [M_f·i + b_{f,1}, ..., M_f·i + b_{f,k}]`
    - `τ_g(i) = [M_g·i + b_{g,1}, ..., M_g·i + b_{g,k}]`
9.  从步骤 4 和 5 中，我们已知：
    - `P([b_{f,1}, ..., b_{f,k}]) = [b_{g,1}, ..., b_{g,k}]` (当 `j=0` 时)
    - `P([M_f·e_j + b_{f,1}, ...]) = [M_g·e_j + b_{g,1}, ...]` (当 `j>0` 时)
    - 这些关系意味着 `P` 不仅置换了偏移向量 `b`，也以相同的方式置换了由矩阵 `M` 产生的项。这可以推导出 `P(M_f·i + b_f) = M_g·i + b_g` 对于列表中的每一个对应元素都成立。
10. `P(τ_f(i)) = P([M_f·i + b_{f,1}, ..., M_f·i + b_{f,k}])`
11. `= [P(M_f·i + b_{f,1}), ..., P(M_f·i + b_{f,k})]` (置换作用于列表中的每个元素)
12. `= [M_g·i + b_{g,1}, ..., M_g·i + b_{g,k}]` (根据步骤 9 的结论)
13. `= τ_g(i)`
14. 这满足了映射置换等价的定义 (3.2)，即 `τ_f ≃_P τ_g`。
15. **证明完毕。**
...
**前提:**
1. `f` 和 `g` 是良构的 SIMD 函数。
2. `∀x, ∀i ∈ I, f(x)[i] = g(x)[i]` (SMT 前提)

**结论:**
`∀x, ∀j, f(x)[j] = g(x)[j]`

**证明:**
1.  根据前提 2，由引理 5.3 可得：`θ_f ≃_Q θ_g`。并且我们知道 `P_0 ∈ Q`。
2.  根据前提 2，由引理 5.4 可得：`τ_f ≃_P τ_g`。并且根据引理 5.5，我们知道这个置换 `P` 就是 `P_0`。
3.  因此，我们有 `P ∈ Q`。
4.  现在，对于任意输入 `x` 和任意索引 `j`，我们来推导 `f(x)[j]`：
5.  `f(x)[j] = θ_f(x[τ_f(j)])`
    (根据 SIMD 函数定义 2.3)
6.  `= θ_g(P(x[τ_f(j)]))`
    (根据步骤 1 和 3，`θ_f ≃_Q θ_g` 且 `P ∈ Q`)
7.  `= θ_g(x[P(τ_f(j))])`
    (根据张量索引性质，`P` 作用于值列表等同于作用于索引列表)
8.  `= θ_g(x[τ_g(j)])`
    (根据步骤 2，`τ_f ≃_P τ_g`)
9.  `= g(x)[j]`
    (根据 SIMD 函数定义 2.3)
10. 因此，`∀x, ∀j, f(x)[j] = g(x)[j]`，证明 `f` 和 `g` 完全等价。
11. **证明完毕。**
